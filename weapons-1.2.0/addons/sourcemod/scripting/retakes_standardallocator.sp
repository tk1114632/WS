...
public Plugin myinfo = {
    ...
    version = "1.1",
    url = "https://github.com/DragonFyZex/csgo-retakes-splewis-convar-weapon-allocator"
};
...
public Action OnClientSayCommand(int client, const char[] command, const char[] args) {
    char gunsChatCommands[][] = { "/gun", "/guns", "gun", "guns", ".gun", ".guns", ".setup", "!weapon", "!weapons", "gnus", "!guns", "!gun" };
...
//T gun round
        	if (!isPistolRound)
        	{
            		int randGiveAwp = GetRandomInt(1, 5);
			...
			{
					int T_Gun_odds = GetRandomInt(1,10);
					if (T_Gun_odds > 8)
					{
						int forced_gun = GetRandomInt(1,4);
						switch (forced_gun) {
							case 1: primary = "weapon_galilar";
							case 2: primary = "weapon_ump45";
							case 3: primary = "weapon_ssg08";
							case 4: primary = "weapon_galilar";
						}
					}
					else
					{
...
//CT gun round
           	...
           	if (giveCTAwp && g_AwpChoice[client] && randGiveAwp == 1 && awp_given < GetConVarInt(g_h_sm_retakes_weapon_awp_team_max)) 
			{
            	...
            	int CT_Gun_odds = GetRandomInt(1,10);
            	if (CT_Gun_odds > 8)
            	{
            		int forced_gun = GetRandomInt(1,4);
					switch (forced_gun) {
						case 1: primary = "weapon_famas";
						case 2: primary = "weapon_ump45";
						case 3: primary = "weapon_ssg08";
						case 4: primary = "weapon_famas";
					}
            	}
...
public void GivePistolMenu(int client) {
	...
	SetMenuTitle(menu, "Select pistol round weapon [手枪局武器]:");
	AddMenuInt(menu, 1, "Glock/P2000/USP-S [带甲/With kevlar]");
	if (GetConVarInt(g_h_sm_retakes_weapon_p250_enabled) == 1)
		AddMenuInt(menu, 2, "p250 [无甲/No kevlar]");
    	if (GetConVarInt(g_h_sm_retakes_weapon_tec9_fiveseven_enabled) == 1)
    		AddMenuInt(menu, 3, "Fiveseven/Tec-9 [无甲/No kevlar]");
    	if (GetConVarInt(g_h_sm_retakes_weapon_cz_enabled) == 1)
       	 	AddMenuInt(menu, 4, "CZ75 [无甲/No kevlar]");
	if (GetConVarInt(g_h_sm_retakes_weapon_cz_enabled) == 1)
       	 	AddMenuInt(menu, 5, "Dual Elite [无甲/No kevlar]");
   	if (GetConVarInt(g_h_sm_retakes_weapon_dual_elite_enabled) == 1)
        	AddMenuInt(menu, 6, "Deagle [无甲/No kevlar]");
...
public void GiveWeaponMenu(int client) {
    ...
    SetMenuTitle(menu, "Select a CT rifle: [CT回防武器]" );
...
public void GiveAwpMenu(int client) {
    ...
    SetMenuTitle(menu, "Allow yourself to receive AWPs? [是否允许获得AWP]");
...
public void GiveSidearmMenu(int client) {
	...
	SetMenuTitle(menu, "Select pistol for gun rounds [长枪局副武器]:");
