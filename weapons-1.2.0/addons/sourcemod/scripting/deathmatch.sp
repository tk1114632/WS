        ...
        for (int i = 0; i < sizeof(menuTriggers); i++)
        {
            if (StrEqual(text, menuTriggers[i], false))
            {
                if (g_cvDM_gun_menu_mode.IntValue == 1 || g_cvDM_gun_menu_mode.IntValue == 2 || g_cvDM_gun_menu_mode.IntValue == 3)
                    ...
                    CPrintToChat(client, "\x04[TG Pro]\x01 %t", "Guns Disabled");
        ...
        if (g_cvDM_headshot_only_allow_client.BoolValue)
        {
            for (int i = 0; i < sizeof(hsOnlyTriggers); i++)
            {
                if (StrEqual(text, hsOnlyTriggers[i], false))
                {
                    ...
                    CPrintToChat(client, "\x04[TG Pro]\x01  %t", buffer);
...
void DisplayOptionsMenu(int client)
{
    ...
    menu.SetTitle("武器菜单/Weapon Menu:");
    menu.AddItem("New", "新武器/New weapons");
    menu.AddItem("Same", "与上局相同武器/Same weapons", allowSameWeapons);
    menu.AddItem("Random", "随机武器/Random weapons");
        ...
        if (IsValidClient(client) && GetClientTeam(client) >= CS_TEAM_T)
        {
            if (!IsFakeClient(client))
            {
                if (g_cvDM_welcomemsg.BoolValue && !g_bInfoMessage[client])
                {
                    ...
                    CPrintToChat(client, "\x04[TG Pro]\x01 This server is running \x04Deathmatch \x01v%s", PLUGIN_VERSION);
                }
                /* Hide radar. */
                ...
                if (!g_bInfoMessage[client])
                {
                    if (g_cvDM_headshot_only.BoolValue)
                        CPrintToChat(client, "\x04[TG Pro]\x01 %t", "HS Only");

                    if (g_cvDM_headshot_only_allow_client.BoolValue)
                        CPrintToChat(client, "\x04[TG Pro]\x01 %t", "HS Only Client");

                    if (g_cvDM_gun_menu_mode.IntValue <= 3)
                        CPrintToChat(client, "\x04[TG Pro]\x01 %t", "Guns Menu");
        ...
        if (validAttacker)
        {
            ...
            if ((knifed && (g_cvDM_hp_knife.IntValue > 0)) || (!knifed && (g_cvDM_hp_kill.IntValue > 0)) || (headshot && (g_cvDM_hp_headshot.IntValue > 0)) || (!headshot && (g_cvDM_hp_kill.IntValue > 0)))
            {
                ...
                if (g_cvDM_hp_messages.BoolValue && !g_cvDM_ap_messages.BoolValue)
                {
                    ...
                    {
                        if (knifed)
                            CPrintToChat(attacker, "\x04[TG Pro]\x01 \x04+%i HP\x01 %t", g_cvDM_hp_knife.IntValue, "HP Knife Kill");
                        else if (headshot)
                            CPrintToChat(attacker, "\x04[TG Pro]\x01 \x04+%i HP\x01 %t", g_cvDM_hp_headshot.IntValue, "HP Headshot Kill");
                        else if (naded || decoy || inferno)
                            CPrintToChat(attacker, "\x04[TG Pro]\x01 \x04+%i HP\x01 %t", g_cvDM_hp_nade.IntValue, "HP Nade Kill");
                        else 
                            CPrintToChat(attacker, "\x04[TG Pro]\x01 \x04+%i HP\x01 %t", g_cvDM_hp_kill.IntValue, "HP Kill");
                    }
                }
            }
            ...
            if ((knifed && (g_cvDM_ap_knife.IntValue > 0)) || (!knifed && (g_cvDM_ap_kill.IntValue > 0)) || (headshot && (g_cvDM_ap_headshot.IntValue > 0)) || (!headshot && (g_cvDM_ap_kill.IntValue > 0)))
            {
                ...
                if (g_cvDM_ap_messages.BoolValue && !g_cvDM_hp_messages.BoolValue)
                {
                    ...
                    {
                        if (knifed)
                            CPrintToChat(attacker, "\x04[TG Pro]\x01 \x04+%i AP\x01 %t", g_cvDM_ap_knife.IntValue, "AP Knife Kill");
                        else if (headshot)
                            CPrintToChat(attacker, "\x04[TG Pro]\x01 \x04+%i AP\x01 %t", g_cvDM_ap_headshot.IntValue, "AP Headshot Kill");
                        else if (naded || decoy || inferno)
                            CPrintToChat(attacker, "\x04[TG Pro]\x01 \x04+%i AP\x01 %t", g_cvDM_ap_nade.IntValue, "AP Nade Kill");
                        else
                            CPrintToChat(attacker, "\x04[TG Pro]\x01 \x04+%i AP\x01 %t", g_cvDM_ap_kill.IntValue, "AP Kill");
                    }
                }
            }
            ...
            if ((g_cvDM_hp_messages.BoolValue && g_cvDM_ap_messages.BoolValue))
            {
                ...
                {
                    if (knifed)
                        CPrintToChat(attacker, "\x04[TG Pro]\x01 \x04+%i HP\x01 & \x04+%i AP\x01 %t", g_cvDM_hp_knife.IntValue, g_cvDM_ap_knife.IntValue, "HP Knife Kill", "AP Knife Kill");
                    else if (headshot)
                        CPrintToChat(attacker, "\x04[TG Pro]\x01 \x04+%i HP\x01 & \x04+%i AP\x01 %t", g_cvDM_hp_headshot.IntValue, g_cvDM_ap_headshot.IntValue, "HP Headshot Kill", "AP Headshot Kill");
                    else if (naded || decoy || inferno)
                        CPrintToChat(attacker, "\x04[TG Pro]\x01 \x04+%i HP\x01 & \x04+%i AP\x01 %t", g_cvDM_hp_nade.IntValue, g_cvDM_ap_nade.IntValue, "HP Nade Kill", "AP Nade Kill");
                    else
                        CPrintToChat(attacker, "\x04[TG Pro]\x01 \x04+%i HP\x01 & \x04+%i AP\x01 %t", g_cvDM_hp_kill.IntValue, g_cvDM_ap_kill.IntValue, "HP Kill", "AP Kill");
...
public int MenuHandler(Menu menu, MenuAction action, int param1, int param2)
{
    ...
    else if (action == MenuAction_Select)
    {
        ...
        if (StrEqual(info, "New"))
        {
            if (g_cvDM_loadout_style.IntValue <= 1)
            {
                if (g_bWeaponsGivenThisRound[param1])
                    CPrintToChat(param1, "\x04[TG Pro]\x01 %t", "Guns New Spawn");
            }
        ...
        else if (StrEqual(info, "Same"))
        {
            ...
            {
                if (g_bWeaponsGivenThisRound[param1])
                    CPrintToChat(param1, "\x04[TG Pro]\x01 %t", "Guns Same Spawn");
            }
            if (g_cvDM_gun_menu_mode.IntValue == 1 || g_cvDM_gun_menu_mode.IntValue == 4)
        ...
        else if (StrEqual(info, "Random"))
        {
            ...
            {
                if (g_bWeaponsGivenThisRound[param1])
                    CPrintToChat(param1, "\x04[TG Pro]\x01 %t", "Guns Random Spawn");
            }
            if (g_cvDM_gun_menu_mode.IntValue == 1 || g_cvDM_gun_menu_mode.IntValue == 4)
            {
...
public Action Command_RespawnAll(int client, int args)
{
    ...
    RespawnAll();
    CPrintToChat(client, "\x04[TG Pro]\x01 All players have been respawned.");
    return Plugin_Handled;
}
    ...
    if (action == MenuAction_Select)
    {
        ...
        if (StrEqual(info, "Edit"))
        {
            ...
            if (g_bInEditMode)
            {
                ...
                CPrintToChat(param1, "\x04[TG Pro]\x01 %t", "Spawn Editor Enabled");
            }
            else
                CPrintToChat(param1, "\x04[TG Pro]\x01 %t", "Spawn Editor Disabled");
        }
        else if (StrEqual(info, "Nearest"))
        {
            ...
            if (spawnPoint != -1)
            {
                ...
                CPrintToChat(param1, "\x04[TG Pro]\x01 %t #%i (%i total).", "Spawn Editor Teleported", spawnPoint + 1, g_iSpawnPointCount);
            }
        }
        else if (StrEqual(info, "Previous"))
        {
            if (g_iSpawnPointCount == 0)
                CPrintToChat(param1, "\x04[TG Pro]\x01 %t", "Spawn Editor No Spawn");
            else
            {
        ...
        else if (StrEqual(info, "Delete"))
        {
            ...
            if (spawnPoint != -1)
            {
                ...
                CPrintToChat(param1, "\x04[TG Pro]\x01 %t #%i (%i total).", "Spawn Editor Deleted Spawn", spawnPoint + 1, g_iSpawnPointCount);
            }
        }
        ...
        {
            if (WriteMapConfig())
                CPrintToChat(param1, "\x04[TG Pro]\x01 %t", "Spawn Editor Config Saved");
            else
                CPrintToChat(param1, "\x04[TG Pro]\x01 %t", "Spawn Editor Config Not Saved");
        }
...
public int PanelConfirmDeleteAllSpawns(Menu menu, MenuAction action, int param1, int param2)
{
    if (action == MenuAction_Select)
    {
        ...
        {
            g_iSpawnPointCount = 0;
            CPrintToChat(param1, "\x04[TG Pro]\x01 %t", "Spawn Editor Deleted All");
        }
        BuildSpawnEditorMenu(param1);
    }
}
...
int GetNearestSpawn(int client)
{
    if (g_iSpawnPointCount == 0)
    {
        CPrintToChat(client, "\x04[TG Pro]\x01 %t", "Spawn Editor No Spawn");
        return -1;
    }
...
void AddSpawn(int client)
{
    ...
    {
        CPrintToChat(client, "\x04[TG Pro]\x01 %t", "Spawn Editor Spawn Not Added");
        return;
    }
    ...
    g_iSpawnPointCount++;
    CPrintToChat(client, "\x04[TG Pro]\x01 %t", "Spawn Editor Spawn Added", g_iSpawnPointCount, g_iSpawnPointCount);
}
...
void InsertSpawn(int client)
{
    ...
    else
    {
        ...
        g_iSpawnPointCount++;
        CPrintToChat(client, "\x04[TG Pro]\x01 %t #%i (%i total).", "Spawn Editor Spawn Inserted", g_iLastEditorSpawnPoint[client] + 1, g_iSpawnPointCount);
    }
}
...
public Action Command_ResetStats(int client, int args)
{
    ...
    ResetSpawnStats();
    CPrintToChat(client, "\x04[TG Pro]\x01 Spawn statistics have been reset.");
    return Plugin_Handled;
}
